...
from utility.utility import lex_solve, solve_lex, solve_lex_batches, general_solve, \
    compute_obj_value, create_batches_weights
...
class FWI():
    ...
    def start_fwi(self):
        ...
        #if self.model.solve():
        if general_solve(self.cache_constraints, self.default_batches, objectives,
                         self.tracker.get_remaining_time(), [], self.solver):
            ...
            non_dominated_solutions, statistics, to_continue = self.fwi_method(self.cache_constraints, objectives,
    ...
    def fwi_method(self, cache_constraints, objectives, variables, current_solution, non_dominated_solutions, depth, to_fix, top_k):
        '''
        ...
        if depth < len(objectives) - 2:
            non_dominated_solutions, statistics, to_continue = self.fwi_method(cache_constraints, objectives, variables,
        ...
        while len(non_dominated_solutions) < top_k and i<len(table_worse_up_to)-1:
            ...
            new_constraints.append(cpmpy.all(clause_fwi))
            ...
            if general_solve(cache_constraints, batch_weights, objectives[depth:],
                             self.tracker.get_remaining_time(), new_constraints,
                             self.solver):
                if depth < len(objectives) - 2:
                ...
                if depth < len(objectives) - 2:
                    ...
                    non_dominated_solutions, statistics, to_continue = self.fwi_method(cache_constraints, objectives,
    ...
    def make_clause_fwi(self, objectives, depth, table_2):
        ...
        '''
        #dictionary = make_dictionary_from_table(table_2)
        #clause_fwi = make_disjunction_fwi(dictionary,objectives[depth+1:])
    ...
    def make_classic_disjunction(self, table,objectives):
        ...
        for row in table:
            ...
            disjunction_classic.append(any(part_disjunction))
        return all(disjunction_classic)
